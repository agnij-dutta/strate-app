...
export default function ConnectButton() {
  const { status, address, connect, disconnect, rehydrate, error } = useWallet();
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="absolute right-0 top-12 z-50 w-56 border border-parchment/12 bg-ink-deep/95 backdrop-blur-md shadow-panel"
              style={{ borderRadius: 2 }}
            >
              <div className="border-b border-parchment/10 px-4 py-3">
                ...
                <p className="mt-1 font-mono text-[11px] text-parchment/90 break-all">
                  {address}
                </p>
              </div>
              <button
  ...
  return (
    <div className="relative">
      ...
      {error && (
        <p className="absolute right-0 top-12 max-w-xs font-mono text-[10px] text-foil/85">
          {error}
        </p>
      )}
    </div>
